                        ...
                        <table class="table table-striped mt-4">
                            ...
                            @foreach($users AS $user)
                                <tr>
                                    ...
                                    <td>{{$user->name}}</td>
                                    <td class="d-flex">
                                        <a class="m-1 btn text-light btn-sm btn-success" href="{{route('user.edit', ['user' => $user->id])}}">Editar</a>
                                        <a class="m-1 btn text-light btn-sm btn-warning" href="">Perfis</a>
                                        <form class="m-1" action="{{route('user.destroy', ['user' => $user->id])}}"  method="post">
                                            @csrf
                                            @method('delete')
